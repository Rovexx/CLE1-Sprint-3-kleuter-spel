...
versie 4.0
- alle bugs gefixed en code getest
- kleuren van teams knipperen als dat team wint
...
LiquidCrystal_I2C lcd1(0x3F, 16, 2);    // scherm met jumper gesoldeerd
LiquidCrystal_I2C lcd2(0x3E, 16, 2);    // scherm standaard
...
int maximumScore = 8;           // de maximale te behalen score
...
unsigned long tijdBegin2;             // gebruikt om de tijd bij te houden
...
void setup() 
{
  ...
  lcd1.print("Haaien");
  lcd1.setCursor(0,1); 
  lcd1.print(scoreTeamLCD[scoreTeam1]);
  
  lcd2.clear();
  lcd2.setCursor(5,0);
  lcd2.print("Leeuwen");
  lcd2.setCursor(0,1); 
  lcd2.print(scoreTeamLCD[scoreTeam1]);
}
...
void timerStopTeam1()
{
  ...
  tijdEind = (tijdEind / 100000);
  tijdTeam1 = (tijdEind - tijdBegin);       // tijd meten van deze ronde
  Serial.println("team1 tijd");
  Serial.println(tijdTeam1);
}
...
void timerStopTeam2()
{
  tijdEind = micros();                    // huidige tijd ophalen
  tijdEind = (tijdEind / 100000);
  tijdTeam2 = (tijdEind - tijdBegin2);       // tijd meten van deze ronde
  Serial.println("begin tijd team 1");
  Serial.println(tijdBegin);
  Serial.println("tijd team 1");
  Serial.println(tijdTeam1);
  Serial.println("Team2 tijd");
  Serial.println(tijdTeam2);
  Serial.println("begin tijd team 2");
  Serial.println(tijdBegin2);
}
...
void scorePrinten()
{
  if (teamGewonnen == 1)                                                        // team 1 heeft gewonnen
  {
    ...
    teamGewonnen = 0;
    delay(1000);
    if (scoreTeam1 == 2)                        // als de maximale score behaald is reset het spel en wordt de kleur van het winnende team knipperend en ronddraaiend op de pixel ring getoond
    {
      iemandGewonnen = true;
  ...
  if (teamGewonnen == maximumScore)                                                      // team 2 heeft gewonnen
  {
    scoreTeam2++;                               // de score van team 1 optellen
    lcd2.clear();
    lcd2.setCursor(5,0);
    lcd2.print("Leeuwen");
    lcd2.setCursor(0,1); 
    lcd2.print(scoreTeamLCD[scoreTeam2]);       // de lcd print een gedeelte van de array met scores omdat we de score van het team elke keer dat ze winnen 1 omhoog doen wordt ook elke keer een volgend gedeelte van de array geprint
    teamGewonnen = 0;
    delay(1000);
    if (scoreTeam2 == maximumScore)                        // als de maximale score behaald is reset het spel en wordt de kleur van het winnende team knipperend en ronddraaiend op de pixel ring getoond
    {
      iemandGewonnen = true;
      scoreTeam1 = 0;                                 // score resetten
      scoreTeam2 = 0;                                 // score resetten
...
{
for (int timer = 0; timer <= 500; timer++)             // 5 seconden keer een kleur van de regenboog tonen
  {
    for (int led = 0; led < NUMPIXELS; led++)
    {
      ...
      pixels.show();
      //delay(1);            // snelheid effect
    }
  }
}

// team 1 blauw knipperen
void team1WinKleur()
{
R = 0;
G = 0;
B = 0;
for (int timer = 0; timer <= 5; timer++)          // 5 seconden kleur van team 1 knipperen
  {
    while (B <= 254)
    {
      for (int led = 0; led < NUMPIXELS; led++)
      {
        pixels.setPixelColor(led, 0, 0, B);         // kleur instellen
      }
      B++;                                         
      pixels.show();                                // strip laten zien
      delay(1);                                   // delay van aan tot uit
    }                                            
    while (B >= 1)
    {
      for (int led = 0; led < NUMPIXELS; led++)
...
// team 2 geel knipperen
void team2WinKleur()
{
R = 0;
G = 0;
B = 0;
for (int timer = 0; timer <= 5; timer++)          // 5 seconden kleur van team 2 knipperen
  {
    while (R <= 254)
    {
      for (int led = 0; led < NUMPIXELS; led++)
      {
        pixels.setPixelColor(led, R, G, 0);         // kleur instellen
      }
      R++;
      G++;
      pixels.show();                                // strip laten zien
